...
class HorarioAtencion(models.Model):
    ...
    def __str__(self):
        return self.dia_semana.title()
    
    @property
    def dia_semana_capitalizado(self):
        return self.dia_semana.capitalize()  # Capitaliza la primera letra
...
class CitaMedica(models.Model):
    ...
    @staticmethod
    def cantidad_citas_por_dia(fecha):
        """
        Retorna la cantidad de citas médicas programadas para un día específico.
        :param fecha: Fecha para la cual contar las citas (tipo datetime.date).
        :return: Entero con la cantidad de citas.
        """
        return CitaMedica.objects.filter(fecha=fecha).count()
    
    @staticmethod
    def proximas_citas():
        """Obtiene las citas del día actual, ordenadas por hora."""
        hoy = date.today()
        return CitaMedica.objects.filter(fecha=hoy).order_by('hora_cita')
